...
#include "../../cayppercore/domainExpansion/jsscriptloader.h"
...
void KDEChanger::setWallpaper(stateOfMons* newState){
    ...
    for(auto const& monitor:newState->name){
        intVectorMon.push_back(std::stoi(monitor));
    }
    json givenVars;
    givenVars["monitors"]=intVectorMon;
    givenVars["wallpaperPath"]="file://"+newState->wallPath.string();
