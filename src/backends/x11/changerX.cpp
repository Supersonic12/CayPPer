...
#include "../../cayppercore/domainExpansion/getConfigPath.h"
#include <nlohmann/json.hpp>
#include <fstream>
using json = nlohmann::json;
void XChanger::setWallpaper(stateOfMons* newState) {
    std::string str_mode;
    if (!newState) {
        throw std::runtime_error("ERROR: state initialized as nullptr");
    }
    if (newState->name.empty()) {
        throw std::runtime_error(std::string("Warning: No Monitor Checked, Check at least one!"));
    }
    if (auto mapped = mapToXWall(newState->fillMode)) {
    ...
    for (const auto& monitor : newState->name) {
        char* argv[] = {
            (char*)"xwallpaper",
            (char*)"--output",
        ...
        if (waitpid(pid, &wstatus, 0) == -1) {
            throw std::runtime_error(
                std::string(
                    "ERROR: waitpid failed while setting wallpaper to monitor: "
                    + monitor));
        }
