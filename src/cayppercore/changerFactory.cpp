...
#include <map>
#include "IChanger.h"
static constexpr const char* BACKEND_DIR="/usr/lib/caypper/";
...
BackendPtr ChangerFactory::loadBackend(const std::string& soName){
    ...
    if(!create || !destroy) {
        dlclose(handle);
        throw std::runtime_error(std::string("ERROR: backend symbols are missin in )"+soName));
    }
    return BackendPtr(
        create(),
...
BackendPtr ChangerFactory::create(EnvVarDetector::Compositor compositor, bool isWayland)
{
    ...
    else
    {
        ...
        if(it!=waylandLibs.end()) libName=it->second;
    }
    if(libName.empty()) throw std::runtime_error(std::string("ERROR: Unknown Compositor."));
